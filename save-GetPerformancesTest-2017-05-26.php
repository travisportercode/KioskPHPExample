...
$_SESSION['startTime'] = date("Y-m-d H:i:s", time()+3600);
$_SESSION['startDate'] = date("Y-m-d", time()+3600);
$_SESSION['endDate'] = date("Y-m-d", time()+3600);
...
if (!empty($_REQUEST['date'])){
    ...
    $_REQUEST['date'] = str_replace("-", "", $_SESSION['startDate']);
    $_SESSION['startDate'] = $_REQUEST['date'];
    $_SESSION['endDate'] = $_REQUEST['date'];
}
...
if (strpos($environmentUrl,"GetPerformancesTest") > 1){
    ...
    $_SESSION['salesCacheFile'] = '/ECommerceSessions/frost/PerformancesTest_' . $_SESSION['startDate'] . '.txt';
    $_SESSION['logFile'] = '/ECommerceSessions/frost/logPerformancesTest_' . $_SESSION['startDate'] . '.txt';
//    $_SESSION['logFile'] = '/Library/WebServer/Documents/logPerformancesTest_' . date('Ymd') . '.txt';
} else {
    $_SESSION['soapUrl'] = 'https://tesslive.frostscience.org/TessituraWebAPI/tessitura.asmx?WSDL';
    $_SESSION['soapUrlDirect'] = 'https://tesslive.frostscience.org/TessituraWebAPI/tessitura.asmx';
    $_SESSION['salesCacheFile'] = '/ECommerceSessions/frost/PerformancesProd_' . $_SESSION['startDate'] . '.txt';
    $_SESSION['logFile'] = '/ECommerceSessions/frost/logPerformancesProd_' . $_SESSION['startDate'] . '.txt';
}
//@sessionkey=KR2QUTLCMURM5BYD91Y2U47UJQRVJR2BUN36M3OF02EVNWIW2SHVWIIEI3T3Q174&@Mstart_date_range=10/05/2016&@Mend_date_range=11/01/2017&@Mmos=3&@Mbu=1
getNewSessionKey();
//$data['localProcedureId'] = '63';
//$data['localProcedureValues'] = '@sessionkey=' . $_SESSION['sessionKey'] . '%26@Mstart_date_range=' . $_SESSION['startDate'] . '%26@Mend_date_range=' . $_SESSION['endDate'] . '%26@Mmos=22&@Mbu=1';
//$performances = getPerformancesChansouda($data);
//$performances = getPerformancesNew($data);
//$performances = getPerformancesOld($data);
$performances = getPerformances();

echo "Performances for : " . $_SESSION['startDate'] . ' based on a time of: ('  . $_SESSION['startTime'] . ')<br><hr><br>';
//exit();
...
function logData($dataToLog){
    ...
    if ($testMode == false){
        ...
        $fh = fopen($_SESSION['logFile'], 'a') or die("can't open file");

        fwrite($fh,  "-----------" . date('m/d/Y H:i:s') . "---------------\n"); 
        fwrite($fh,$dataToLog . "\n");
        fwrite($fh,  "----------------------------------------------------------------------------------------\n\n\n\n\n\n\n");
        fclose($fh);
    }
...
//@sessionkey=KR2QUTLCMURM5BYD91Y2U47UJQRVJR2BUN36M3OF02EVNWIW2SHVWIIEI3T3Q174&@Mstart_date_range=10/05/2016&@Mend_date_range=11/01/2017&@Mmos=3&@Mbu=1
function getPerformancesNew($data){
    $ch = curl_init(); 
...
//    exit();
    ...
    $startPos = strpos($response, "<LocalProcedure");
    $endPos = strrpos($response, "</LocalProcedure>");
    //$_SESSION['sessionKey'] = substr($response,$startPos,$endPos - $startPos);
    //$response = "<Performances>\n" . substr($response,$startPos,$endPos-$startPos) . "</Performances>\n"; // get rid of everything else.
    $response = substr($response,$startPos,$endPos-$startPos) . "</LocalProcedure>"; // get rid of everything else.
...
//    exit;
    ...
    foreach($responseLines as $oneResponseLine){
        if (strstr($oneResponseLine,"<LocalProcedure diff")){
            $response .= "<Performance>" . "\n";
        } else if (strstr($oneResponseLine,"</LocalProcedure>")){  //csn added 20161021
            $response .= "</Performance>" . "\n";    
        } else {
...
//        echo "</textarea>";
//        exit;
        $onSaleInd = (string) $onePerformanceSimpleObject->on_sale_ind;
        if ($onSaleInd == 'Y'){
            ...
            } else if ($performance->description == 'mfaNOW'){
                $performance->mainCategory = 'ga'; 
                $performance->pricingGroup = 'Museum Admission'; // put in the performance description that the prices should appear under.
            ...
            $words = explode("T",$performance->perfDateTime);
            $performanceDate = str_replace("-", "", $words[0]);
            
            if ($performanceDate == $_SESSION['startDate']){
                ...
                if ($performance->rawPricesResponse == null) {
...
//        exit('price group:' . $performance[$i]['priceGroup']);
...
function getPerformances(){
     $url = $_SESSION['soapUrlDirect'] . '/GetPerformancesEx?'
             ...
             . 'sArtistLastName=';
     //url = https://tesslive.frostscience.org/TessituraWebAPI/tessitura.asmx/GetPerformancesEx?sWebSessionId=&sStartDate=20170504&sEndDate=20170504&iVenueID=-1&iModeOfSale=9&iBusinessUnit=-1&sSortString=&sKeywords=&cKeywordAndOrStatement=&sArtistLastName=
//    exit("url = " . $url);
...
//    exit();
    ...
    foreach($simpleObject->AllPrice as $oneSimpleAllPrice){
        ...
        $price = new Price();
        $price->price_type_desc = (string) $oneSimpleAllPrice->price_type_desc;
        ...
        $price->description = (string) $oneSimpleAllPrice->description;
        //$price->priceGroup = (string) $oneSimpleAllPrice->price_group;
//        echo "<pre>";
//        print_r($oneSimpleAllPrice);
//        echo "</pre>";
//        exit;
